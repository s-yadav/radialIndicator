        ...
        return function (num) {
            ...
            if (pattern.includes('.')) {
                ...
                const numSplit = number.toString().split('.'),
                    digits = replaceHashes(patternDigitsDecimals[0], parseFloat(numSplit[0]).toFixed(0)),
                    decimals = replaceHashes(patternDigitsDecimals[1], numSplit[1]);
    ...
    Indicator.prototype = {
        ...
        animate: function (val, animationDuration) {
            ...
            const back = val < counter,
                valDelta = Math.abs(this.current_value - val),
                minMaxDelta = Math.abs(maxVal - minVal),
                valDeltaMultiplier = valDelta / minMaxDelta,
                calcDuration = animationDuration || indOption.duration * valDeltaMultiplier || indOption.frameTime * frameNum * valDeltaMultiplier; // frameTime legacysupport
            let start;

            const processAnimation = now => {
                if (!start) start = now;
                const runtime = now - start,
                    progress = runtime / calcDuration,
                    step = valDelta * indOption.easing(progress);
